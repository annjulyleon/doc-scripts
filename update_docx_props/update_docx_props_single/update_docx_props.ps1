...
.Example
	.\UpdateDocxPropsConfig_v3.ps1 -dir D:\projects\myproject\ -conf D:\projects\myproject\config.xml -filename includethisfile.docx
.PARAMETER
...
Else {
    [array]$files = @(Get-ChildItem -Path $path -Include *.doc, *.docx -Recurse -File | select -expand fullname | Where-Object {$_.PSParentPath -notmatch $exclude}) 
